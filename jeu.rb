class Personne
  attr_accessor :nom, :points_de_vie, :en_vie

  def initialize(nom)
    @nom = nom
    @points_de_vie = 100
    @en_vie = true
  end

  def info
    # A faire:
    if en_vie
      # - Renvoie le nom et les points de vie si la personne est en vie
      return "#{nom} (#{points_de_vie}/100 points de vie)"
    else
      # - Renvoie le nom et "vaincu" si la personne a été vaincue
      return "#{nom} est vaincu !"
    end
  end

  def attaque(personne)
    # A faire:
    # - Fait subir des dégats à la personne passée en paramètre
    personne.subit_attaque(self.degats)
    
    # - Affiche ce qu'il s'est passé
    puts "#{personne.nom} s'est fait attaqué par #{nom} !\n\n"
  end

  def subit_attaque(degats_recus)
    # A faire:
    # - Réduit les points de vie en fonction des dégats reçus
    @points_de_vie -= degats_recus
    
    # - Affiche ce qu'il s'est passé
    puts "#{nom} reçoit #{degats_recus} points de dégats"
    
    # - Détermine si la personne est toujours en_vie ou non
    if points_de_vie <= 0
      @en_vie = false
      puts "#{nom} est mort !"
    end
  end
end

class Joueur < Personne
  attr_accessor :degats_bonus

  def initialize(nom)
    # Par défaut le joueur n'a pas de dégats bonus
    @degats_bonus = 0

    # Appelle le "initialize" de la classe mère (Personne)
    super(nom)
  end

  def degats
    # A faire:
    # - Calculer les dégats
    degats = rand(5..30)
    if degats_bonus != 0
      degats += degats_bonus
      @degats_bonus = 0
    end
    
    # - Affiche ce qu'il s'est passé
    if degats < 30
      puts "\n#{nom} attaque !"
    else
      puts "\n#{nom} est déchainé et lance son attaque chargée"
    end
    
    return degats
    
  end

  def soin
    # A faire:
    # - Gagner de la vie
    @points_de_vie += 50
    if @points_de_vie > 100
      @points_de_vie = 100
    end
    # - Affiche ce qu'il s'est passé
    puts "\n#{nom} boit une potion de soin et récupère 50 pv !\n"
  end

  def ameliorer_degats
    # A faire:
    # - Augmenter les dégats bonus
    @degats_bonus += 30
    
    # - Affiche ce qu'il s'est passé
    puts "\n#{nom} charge son attaque (30 points de dégats supplémentaires au prochain tour).\n"
  end
end

class Ennemi < Personne
  def degats
    # A faire:
    # - Calculer les dégats
    return rand(1..10)
  end
end

class Jeu
  def self.actions_possibles(monde)
    puts "ACTIONS POSSIBLES :"

    puts "0 - Se soigner"
    puts "1 - Améliorer son attaque"

    # On commence à 2 car 0 et 1 sont réservés pour les actions
    # de soin et d'amélioration d'attaque
    i = 2
    monde.ennemis.each do |ennemi|
      puts "#{i} - Attaquer #{ennemi.info}"
      i = i + 1
    end
    puts "99 - Quitter"
  end

  def self.est_fini(joueur, monde)
    # A faire:
    # - Déterminer la condition de fin du jeu
    if joueur.en_vie == false || monde.ennemis.size == 0
      return true
    end
  end
end

class Monde
  attr_accessor :ennemis

  def ennemis_en_vie
    # A faire:
    # - Ne retourner que les ennemis en vie
    ennemis.each do |ennemi|
      if ennemi.en_vie == false
        ennemis.delete(ennemi)
      end
    end
    
    return ennemis
  end
end

##############

# Initialisation du monde
monde = Monde.new

# Ajout des ennemis
monde.ennemis = [
  Ennemi.new("Balrog"),
  Ennemi.new("Goblin"),
  Ennemi.new("Squelette")
]

# Initialisation du joueur
joueur = Joueur.new("Jean-Michel Paladin")

# Message d'introduction. \n signifie "retour à la ligne"
puts "\n\nAinsi débutent les aventures de #{joueur.nom}\n\n"

# Boucle de jeu principale
100.times do |tour|
  puts "\n------------------ Tour numéro #{tour} ------------------"

  # Affiche les différentes actions possibles
  Jeu.actions_possibles(monde)

  puts "\nQUELLE ACTION FAIRE ?"
  # On range dans la variable "choix" ce que l'utilisateur renseigne
  choix = gets.chomp.to_i

  # En fonction du choix on appelle différentes méthodes sur le joueur
  if choix == 0
    joueur.soin
  elsif choix == 1
    joueur.ameliorer_degats
  elsif choix == 99
    # On quitte la boucle de jeu si on a choisi
    # 99 qui veut dire "quitter"
    break
  else
    # Choix - 2 car nous avons commencé à compter à partir de 2
    # car les choix 0 et 1 étaient réservés pour le soin et
    # l'amélioration d'attaque
    ennemi_a_attaquer = monde.ennemis[choix - 2]
    joueur.attaque(ennemi_a_attaquer)
  end

  puts "\nLES ENNEMIS RIPOSTENT !"
  # Pour tous les ennemis en vie ...
  monde.ennemis_en_vie.each do |ennemi|
    # ... le héro subit une attaque.
    ennemi.attaque(joueur)
  end

  puts "\nEtat du héro: #{joueur.info}\n"

  # Si le jeu est fini, on interompt la boucle
  break if Jeu.est_fini(joueur, monde)
end

puts "\nGame Over!\n"

# A faire:
# - Afficher le résultat de la partie

if joueur.en_vie
  puts "Après sa périlleuse aventure, #{joueur.nom} sort victorieux de ses combats et rentre chez lui."
  puts "De retour au bercail, le roi le sacre chevalier et #{joueur.nom} épouse enfin la princesse."
  puts "Ils vécurent heureux et eurent beaucoup d'enfants."
else
  puts "Malgré sa bravoure, #{joueur.nom} trépassa lors de son combat contre le mal."
  puts "La nouvelle fait le tour du royaume et tous les badauds sont en deuil."
  puts "Avec le temps, les forces du mal se répandent sur la Terre..."
  puts "Qui pourra remplacer #{joueur.nom} ?"
end